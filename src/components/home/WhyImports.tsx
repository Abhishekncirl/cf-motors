    ...
    <section className="bg-night py-16 text-white">
      <div className="container-page">
        <div className="max-w-2xl">
          <p className="mb-2 text-xs font-semibold uppercase tracking-[0.2em] text-brand-cyan">Our difference</p>
        ...
        <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {POINTS.map(({ icon: Icon, title, body }) => (
            <div key={title} className="rounded-xl border border-white/10 bg-nightcard p-6">
              <span className="inline-flex h-11 w-11 items-center justify-center rounded-lg bg-brand-cyan/15 text-brand-cyan">
        ...
        <div className="mt-8">
          <Link to="/import-service" className="btn-outline-dark">How our import service works</Link>
        </div>
      </div>
    </section>
  );
