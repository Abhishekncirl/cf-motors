...
import { Mail, Phone, Check, Dot, Trash2 } from 'lucide-react';
import { listEnquiries, setEnquiryFlags, deleteEnquiry } from '../../lib/enquiries';
...
// Payload keys that hold arrays of image URLs (rendered as photos, not text).
                ...
                <div className="border-t border-white/10 bg-brand-black/40 p-4">
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    ...
                    <button
                      ...
                      className="btn ml-auto border border-red-400/40 px-3 py-1.5 text-xs text-red-300 hover:border-red-400 hover:text-red-200"
                    >
...
function collectPhotos(payload: Record<string, unknown>): string[] {
  const urls: string[] = [];
  for (const key of PHOTO_KEYS) {
    const v = payload[key];
    if (Array.isArray(v)) {
      for (const item of v) if (typeof item === 'string' && item.startsWith('http')) urls.push(item);
    }
  }
  return urls;
}
