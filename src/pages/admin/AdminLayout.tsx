...
export function AdminLayout() {
  ...
  return (
    <div className="min-h-screen bg-page lg:grid lg:grid-cols-[240px_1fr]">
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-60 transform border-r border-white/10 bg-night text-white transition-transform lg:static lg:translate-x-0 ${
      ...
      >
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 rounded-md px-3 py-2.5 text-sm font-semibold ${
                  isActive ? 'bg-brand-cyan text-brand-black' : 'text-white/80 hover:bg-white/5'
                }`
              }
            >
        ...
        <div className="absolute inset-x-0 bottom-0 space-y-2 border-t border-white/10 p-3">
          <Link to="/" target="_blank" className="flex items-center gap-2 rounded-md px-3 py-2 text-xs text-white/60 hover:text-brand-cyan">
            <ExternalLink size={14} aria-hidden /> View live site
          </Link>
          <button onClick={onLogout} className="flex w-full items-center gap-2 rounded-md px-3 py-2 text-sm text-white/80 hover:bg-white/5">
