...
export function AdminLayout() {
  ...
  return (
    <div className="min-h-screen bg-page lg:grid lg:grid-cols-[240px_1fr]">
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-60 transform border-r border-line bg-white transition-transform lg:static lg:translate-x-0 ${
      ...
      >
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 rounded-md px-3 py-2.5 text-sm font-semibold ${
                  isActive ? 'bg-brand-cyan text-brand-black' : 'text-ink/80 hover:bg-black/5'
                }`
              }
            >
        ...
        <div className="absolute inset-x-0 bottom-0 space-y-2 border-t border-line p-3">
          <Link to="/" target="_blank" className="flex items-center gap-2 rounded-md px-3 py-2 text-xs text-ink/60 hover:text-teal">
            <ExternalLink size={14} aria-hidden /> View live site
          </Link>
          <button onClick={onLogout} className="flex w-full items-center gap-2 rounded-md px-3 py-2 text-sm text-ink/80 hover:bg-black/5">
